...
confhome_cn=https://cnb.cool/wiyadinarulita8048/gileredman/-/git/raw/main
# confhome_cn=https://www.ghproxy.cc/https://raw.githubusercontent.com/wiyadinarulita8048/reinstall/main
...
Manual: https://github.com/wiyadinarulita8048/reinstall
...
download_and_extract_apk() {
    ...
    if [ -z "$package_apk" ] || ! [ "$(wc -l <<<"$package_apk")" -eq 1 ]; then
...
mod_initrd_alpine() {
    ...
    if ! insert_into_file init after 'configure_ip\(\)' <<EOF
        depmod
...
EOF
    then
        error_and_exit "configure_ip() not found in Alpine init — network setup injection failed"
    fi
...
EOF
    ...
    # hack 5 guard before switch_root: ensure /sbin/init exists in the new root
    # If Alpine apk/setup failed (e.g. repo fetch error), sysroot will be empty
    # and the default "not found" message is confusing. Give a clear diagnostic.
    insert_into_file init before '^exec (/bin/busybox )?switch_root' <<EOF
        if [ ! -x \$sysroot/sbin/init ] && [ ! -x \$sysroot/bin/init ]; then
            echo "ERROR: /sbin/init not found in new root -- Alpine setup likely failed"
            echo "ERROR: check network connectivity and repository access in initramfs"
            exec sh
        fi
EOF
...
脚本有更新，请重新下载 reinstall.sh"
    ...
    [ -s "$initrd_dir/initrd-network.sh" ] || error_and_exit "Failed to download initrd-network.sh from $confhome"
